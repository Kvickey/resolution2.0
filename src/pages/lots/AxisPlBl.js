...
const AxisPlBl = ({
  ...
  const createRecordObject = (row, borrowerArray) => ({
    ...
    Borrower: borrowerArray.map((b) => ({
      ...
      Work_mobile_no: v(b.WORK_MOBILE_2),
      Email_id: v(b.E_MAIL_ID),
      Comm_add: v(b.Communication_address),
      And_also_at: v(b.And_Also_At_address1),
      And_also_at2: v(b.And_Also_At_address2),
      And_also_at3: v(b.And_Also_At_address3),
      Work_add: v(b.work_Address),
    })),
  ...
  const handleVerify = async () => {
    ...
    for (let i = 0; i < excelData.length; i++) {
      ...
      if (row.REFERENCE_NO !== prevRef && borrowers.length) {
        await verifyData(createRecordObject(excelData[i - 1], borrowers), i - 1);
      ...
      borrowers.push({
        ...
        Comm_add: row.COMM_ADDRESS,
        And_also_at: row.ALSO_AT_1,
        And_also_at2: row.ALSO_AT_2,
        And_also_at3: row.ALSO_AT_3,
