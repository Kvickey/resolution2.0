...
const SOC = () => {
  const [socNotCreatedLots, setSocNotCreatedLots] = useState([]);
  ...
  useEffect(() => {
    const fetchSocNotCreatedLots = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/api/pendingSoc`);
        if (!response.ok) {
        ...
        setSocNotCreatedLots(parsedNotServedLots);
        setLoading(false);
      } catch (error) {
    ...
    fetchSocNotCreatedLots();
  }, []);

  console.log(socNotCreatedLots);
  ...
  const totalPages = Math.ceil(socNotCreatedLots.length / 10); // Example calculation
  ...
  const steps = ["Select Lot", "Save SOC", "Generate SOC", "Upload SOC"];
  ...
  const handleSaveSOC = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/SocSave`, {
        method: "POST",
        headers: {
      ...
      {!showData && (
        <div className="row">
          <div className="col-md-12 mt-3">
            <ReusableTableFixed
              columns={columns}
              data={socNotCreatedLots.slice(startIndex, startIndex + 10)}
      ...
      {clearForm && (
        <div className="row">
          <div className="col-md-12 d-flex justify-content-center ">
            <ClearForm
              message="SOC Letter Uploaded Successfully!"
