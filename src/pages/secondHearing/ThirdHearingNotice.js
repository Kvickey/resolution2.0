...
const ThirdHearingNoticee = () => {
  ...
  const handleGenerateAcceptance = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/ThirdHearingletter?Lot_no=${selectedLotNo}&Client_id=${selectedClientID}&Product_id=${selectedProductID}&Arb_id=${arbId}&Meeting_no=2&SH_date=${secondHearingDate}`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
...
export default ThirdHearingNoticee;
