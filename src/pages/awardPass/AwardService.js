...
const AwardService = () => {
    ...
    const fetchNotServedLots = async () => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/notServed?s_id=8&Arb_id=${arbId}`
        );
        if (!response.ok) {
  ...
  const handleData = async (lot) => {
    // console.log(lot);
    setLoading(true);
    const url = `${API_BASE_URL}/api/notServed?s_id=1&Lot_no=${lot}&arb_id=${arbId}`;
    console.log(url);
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/notServed?s_id=8&Lot_no=${lot}&arb_id=${arbId}`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
